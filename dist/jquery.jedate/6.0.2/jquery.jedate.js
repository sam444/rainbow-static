...
;(function(root, factory) {
    ...
    var jet = {}, doc = document, regymdzz = "YYYY|MM|DD|hh|mm|ss|zz", gr = /\-/g,
        ...
        config = {
            ...
            maxDate:"2099-12-31 24:59:59" //最大日期
        };
    ...
    jedfn.initdates = function(){
        ...
        var formatDate = function (cls,boxcell) {
            ...
            dateDiv.attr("author","tony").css({"z-index": boxcell != "#jedatebox" ? "" : zIndex ,"position":reabsfix});
    ...
    jedfn.eachDays = function(ys, ms,ds, opts, boxCell){
        ...
        var setMark = function (my, mm, md) {
            ...
            return $.isArray(Marks) && Marks.length > 0 ? '<i class="marks"></i>' :"";
        };
        var setWork = function (my, mm, md) {
            var Works = opts.works, contains = function(arr, obj) {
                var len = arr.length;
                while (len--) {
                    if (arr[len] === obj) return true;
                }
            ...
            return $.isArray(Works) && Works.length > 0 ? '<i class="works"></i>' :"";
        };
        //是否显示节日
        var isfestival = function(y, m ,d) {
            ...
            if(opts.festival == true && lang.name == "cn"){
                var lunar = that.jeLunar(y, m - 1, d), feslunar = (lunar.solarFestival || lunar.lunarFestival),
                    ...
                    lunartext = (feslunar && lunar.jieqi) != "" ? feslunar : (lunar.lunarFestival);
                    var festival = lunar.lunarFestival?"festival":'';
                    
                festivalStr = '<p><span class="solar">' + d + '</span><span class="lunar '+festival+'">' + lunartext + '</span></p>';
            }else{
        ...
        var eachDays = function (yd,md) {
            ...
            for(var b = 1; b <= daysNum; b++, count++){
                var bmark = setMark(yd,md,b), cls = "",ban = setWork(yd,md,b);
                ...
                daysArr.push({style:cls,ymd:yd+'-'+md+'-'+b,day:b,d:(isfestival(yd,md,b) + bmark + ban),idx:didx++});
            }
            //下一月开始天数
        ...
        $.each(new Array(ranges ? 1 : 2),function (m) {
            ...
            $.each([25, 60, 60],function (i,lens) {
                ...
                for (var h = 0; h < lens; h++) {
                    if(opts.minutes&&opts.minutes.length>0){
                        if(i==1){
                            h = opts.minutes[j++];
                        }
                        if(j>opts.minutes.length){
                            break;
                        }
                    }
                    if(opts.seconds&&opts.seconds.length>0){
                        if(i==2){
                            h = opts.seconds[k++];
                        }
                        if(k>opts.seconds.length){
                            break;
                        }
                    }
                    var tlis = $("<li/>");
