        ...
        set_node_color:function(nodeid, bgcolor, fgcolor){
            if(this.get_editable()){
                ...
                if(!!node){
                    ...
                    if(!!fgcolor){
                        node.data['foregroundcolor'] = fgcolor;
                    }
                    this.view.reset_node_custom_style(node);
                }
            }else{
...
// ============= data provider =============================================
    ...
    jm.view_provider.prototype={
        ...
        _reset_node_custom_style:function(node_element, node_data){
            ...
            if('foregroundcolor' in node_data){
                node_element.style.color = node_data['foregroundcolor'];
            }
    ...
    jm.shortcut_provider.prototype = {
        init : function(){
            ...
            this.handles['expand'] = this.handle_expand;
        ...
        handle_expand:function(_jm,e){
            var selected_node = _jm.get_selected_node();
            console.log(selected_node);
            if(selected_node.expanded==false){
                _jm.expand_node(selected_node);
            }else{
                _jm.collapse_node(selected_node);
            }
        ...
        handle_up:function(_jm,e){
            ...
            if(!!selected_node){
                ...
                if(!!up_node){
                    ...
                    var nodeheight=up_node._data.view.abs_y;
                    var scrollheight=document.documentElement.scrollTop;
                    if((nodeheight-scrollheight)<windowHeight/2-200){
                        window.scrollTo(0,nodeheight-windowHeight/2+150);
                    }
                }
                evt.stopPropagation();
                evt.preventDefault();
        ...
        handle_down:function(_jm,e){
            ...
            if(!!selected_node){
                ...
                if(!!down_node){
                    ...
                    var windowHeight=window.screen.availHeight;
                    var nodeheight=down_node._data.view.abs_y;
                    var scrollheight=document.documentElement.scrollTop;
                    if((nodeheight+40-scrollheight)>windowHeight/2){
                       window.scrollTo(0,windowHeight/2+scrollheight-100);
                    }
                }
                evt.stopPropagation();
                evt.preventDefault();
        ...
        _handle_direction:function(_jm,e,d){
            ...
            if(!!selected_node){
                if(selected_node.isroot){
                    ...
                    if(node.isroot){
                        var windowHeight=window.screen.availHeight;
                        var nodeheight=node._data.view.abs_y;
                        var scrollheight=document.documentElement.scrollTop;
                        if((nodeheight-scrollheight)<100||(nodeheight-scrollheight)>windowHeight-200){
                             window.scrollTo(0,nodeheight/2+200);
                        }
                    }
                }
                if(!!node){
